...
<div id="cart-orders" class="uk-form uk-form-horizontal" v-cloak>
	...
	<div class="uk-flex uk-flex-middle uk-flex-wrap" data-uk-margin>
		...
		<h2 class="uk-margin-remove" v-show="selected.length">{{ '{1} %count% Order selected|]1,Inf[ %count% Orders selected' | transChoice selected.length {count:selected.length} }}</h2>

		<div class="uk-margin-left" v-show="selected.length">
			<ul class="uk-subnav pk-subnav-icon">
				<li><a class="pk-icon-delete pk-icon-hover" title="{{ 'Delete' | trans }}"
					   data-uk-tooltip="{delay: 500}" v-on="click: removeOrders"
	...
	<div class="uk-overflow-container">
		<table class="uk-table uk-table-hover uk-table-middle">
			...
			<tbody>
			<tr class="check-item" v-repeat="order: orders" v-class="uk-active: active(order)">
				<td><input type="checkbox" name="id" value="{{ order.id }}"></td>
				<td>
					<a v-attr="href: $url.route('admin/cart/order/edit', { id: order.id })">{{ order.transaction_id }}</a>
				</td>
				<td class="uk-text-center">
					<a title="{{ getStatusText(order) }}" v-class="
